...
const updateRightVertFilter = function(yScaled){
  ...
  if (this.filters.reverb){
    ...
    this.filters.reverb.gain = yScaled / 200;
  }
  else {
    console.log("gong to send ajax request");
    let reverbNode = this.audioContext.createConvolver();
    
    //asynchronously fetch and read audio file for impulse response
    let ajaxRequest = new XMLHttpRequest();
    let crowdBuffer;
    ajaxRequest.open('GET', 'https://s3.amazonaws.com/kleos-dev/crowd.wav');
    ...
    ajaxRequest.onload = () => {
      ...
      this.audioContext.decodeAudioData(audioData, (buffer) => {
        ...
        console.log(crowdBuffer);
        reverbNode.buffer = crowdBuffer;
        let gain = yScaled / 200;
        let filter = {
          node: reverbNode,
