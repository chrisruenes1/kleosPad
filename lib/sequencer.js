...
const Sequencer = function(){
  ...
  //for limiting data flow from echo param
  this.lastEchoUpdate = 0;
...
const updateBottomHorizFilter = function(xScaled){
  ...
  if (this.filterNodes.distortion){
    this.filterNodes.distortion.curve = generateDistortionCurve(xScaled);
  }
  else {
    let distortion = this.audioContext.createWaveShaper();
    distortion.curve = generateDistortionCurve(xScaled);
    distortion.oversample = "4x";
    this.filterNodes.distortion = distortion;
  }
};

const updateTopHorizFilter = function(xScaled){
  //only update every 50ms to avoid whacky space sounds
  if (this.audioContext.currentTime > (this.lastEchoUpdate - 500)){
    let delayTime = (xScaled/200).toPrecision(1); //0-1 seconds delay

    if (this.filterNodes.echo){
      this.filterNodes.echo.delayTime.value = delayTime;
    }
    else {
      let echo = this.audioContext.createDelay();
      echo.delayTime.value = delayTime;
      this.filterNodes.echo = echo;
    }
    this.lastEchoUpdate = this.audioContext.currentTime;
  }
};
