...
/***/ function(module, exports, __webpack_require__) {
	...
	const draw = function(){
	  ...
	  pattern.forEach( (drums, idx) => {
	    ...
	    drums.forEach( (drum) => {
	      ...
	      if (this.sequencer.filters.distortion && drum.playing && this.xScaled > 20){ //
	        ...
	        let max = Math.ceil( ((this.canvas.width - this.pageX) / 1000 ) * 50) + 2; //targeting ~2 - 52 max
	        let min = 1;
	  ...
	  if (this.sequencer.filters.distortion){
	    ...
	    for (let i = 0; i < 3; i++){
	      ...
	      if (this.distortionRenders[i][this.waveIndex] &&
	        ...
	        if (!currentWave){
	          ...
	          console.log("STARTING AT " + currentWave.points[0]);
	...
	const generateWave = function(startTime){
	  ...
	  let r = Math.floor(darkBlue.r + ((lightBlue.r - darkBlue.r) * (this.xScaled / 100)));
	  let g = Math.floor(darkBlue.g + ((lightBlue.g - darkBlue.g) * (this.xScaled / 100)));
	  let b = Math.floor(darkBlue.b + ((lightBlue.b - darkBlue.b) * (this.xScaled / 100)));
	...
	const generateWavePoints = function(){
	  ...
	  let minY = (this.canvas.height/2) - ( (this.canvas.height/2) * (this.xScaled/100) );
	  let maxY = (this.canvas.height/2) + ( (this.canvas.height/2) * (this.xScaled/100) );
