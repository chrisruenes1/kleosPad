...
/***/ function(module, exports, __webpack_require__) {
	...
	const Sequencer = function(){
	  ...
	  let goingHome = Patterns.goingHome.map( (hits) => {
	  ...
	  this.patterns = {"goingHome": goingHome};
	  this.currentPattern = goingHome;
	  this.currentPatternName= "goingHome";
	  this.callbacks = {};
	...
	Sequencer.prototype.registerCallback = function(name, func){
	...
	Sequencer.prototype.getPatternName = function(){
	...
	const scheduleNoteEvents = function( beatNumber, time ) {
	  if (!this.paused){
	    let hits = this.currentPattern[beatNumber];
	    // let filters = [];
	    // for (let key in this.filters){
	    //   filters.push(this.filters[key]);
	    // }
	    hits.forEach( (hit) => {
	      let filters = getFilters.bind(this)(hit);
	...
	const getFilters = function(hit){
	  let filters = [];
	  if (hit.fxScaledPos){
	    filters.push(updateLeftVertFilter.call(this, hit.fxScaledPos[1]));
	    filters.push(updateRightVertFilter.call(this, hit.fxScaledPos[1]));
	    filters.push(updateBottomHorizFilter.call(this, hit.fxScaledPos[0]));
	    filters.push(updateTopHorizFilter.call(this, hit.fxScaledPos[0]));
	  }
	  return filters;
	};
	...
	const setPattern = function(patternName){
	  if (this.patterns[patternName]){
	    ...
	    let currentPattern = Patterns[patternName].map( (hits) => {
	      let drumObjects = hits.map( (drumName) => {
	        let DrumClass = INSTRUMENTS[drumName];
	        return new DrumClass();
	      });
	    ...
	    this.patterns[patternName] = currentPattern;
	    this.currentPattern = currentPattern;
	  }
	  this.currentPatternName = patternName;
	  this.callbacks.patternChange(patternName);
...
/***/ function(module, exports) {
	...
	const Util = {
	  ...
	  getSelectedDrumInCanvas(drumPositions, canvas, event){
	    ...
	    let foundIdx = -1;
	    drumPositions.find( (drumPos, idx) => {
...
/***/ function(module, exports, __webpack_require__) {
	...
	Snare.prototype.attack = function(startTime, filterNodes) {
	  ...
	  let when = startTime > this.currentTime() ? startTime - this.currentTime() : 0;
	...
	Snare.prototype.release = function() {
	  ...
	  this.oscillator.stop(time + this.hitDuration);
...
/***/ function(module, exports, __webpack_require__) {
	...
	const Visualizer = function(sequencer, canvas){
	  ...
	  this.dragging = false;
	  let patternName = this.sequencer.getPatternName();
	  this.drumPositions = {};
	  this.drumPositions[patternName] = [];
	  ...
	  this.sequencer.registerCallback("patternChange", onPatternChange.bind(this));
	...
	const onPatternChange = function(patternName) {
	  if (!this.drumPositions.hasOwnProperty(patternName)){
	    this.drumPositions[patternName] = [];
	  }
	};
	...
	const draw = function(){
	  ...
	  let pattern = this.sequencer.currentPattern;
	  ...
	  pattern.forEach( (drums, idx) => {
	    ...
	    drums.forEach( (drum, subIdx) => {
	      ...
	      drum.setPos(pos);
	      if (!this.drumPositions[patternName].find( (drumPos) => {
	        if (drumPos.beatNum === idx && drumPos.drumNum === subIdx){
	          return true;
	        }
	        else {
	          return false;
	        ...
	        this.drumPositions[patternName].push({
	          pos: pos,
	          rad: this.radius,
	          beatNum: idx,
	          drumNum: subIdx
	...
	const setDragImage = function(e){
	  ...
	  let selectedDrumIdx = Util.getSelectedDrumInCanvas.bind(this)(this.drumPositions[patternName], this.canvas, e);
	  if (selectedDrumIdx >= 0){   
	    $(this.canvas).addClass('pointer');
	    if (this.dragging) {
	      //TODO this really isn't great. Try to remove the duplication
	      let selectedDrum = this.drumPositions[patternName][selectedDrumIdx];
	      selectedDrum.pos = [coords.x, coords.y];
	      
	      let xScaled = (coords.x * 100)/this.canvas.width;
	      let yScaled = (coords.y * 100)/this.canvas.height;
	      this.sequencer.currentPattern[selectedDrum.beatNum][selectedDrum.drumNum].fxScaledPos = [xScaled, yScaled];
	      
	      this.sequencer.currentPattern[selectedDrum.beatNum][selectedDrum.drumNum].setPos([coords.x, coords.y]);
	    }
	  }
	  else {
