...
/***/ function(module, exports, __webpack_require__) {
	...
	const Visualizer = function(sequencer, canvas){
	  ...
	  this.sequencer.registerCallback("patternChange", onPatternChange);
	...
	const onPatternChange = function(){
	  console.log("onPatternChange called");
	  this.drumPositions = [];
	};
	...
	const draw = function(){
	  ...
	  pattern.forEach( (drums, idx) => {
	    ...
	    drums.forEach( (drum, subIdx) => {
	      ...
	      drum.setPos(pos);
	      if (!this.drumPositions.find( (drumPos) => {
	...
	const setDragImage = function(e){
	  ...
	  let selectedDrumIdx = Util.getSelectedDrumInCanvas.bind(this)(this.drumPositions, this.canvas, e);
	  if (selectedDrumIdx >= 0){   
	    $(this.canvas).addClass('pointer');
	    if (this.dragging) {
	      //TODO this really isn't great. Try to remove the duplication
	      let selectedDrum = this.drumPositions[selectedDrumIdx];
	      selectedDrum.pos = [coords.x, coords.y];
	      this.sequencer.pattern[selectedDrum.beatNum][selectedDrum.drumNum].setPos([coords.x, coords.y]);
	    }
	  }
	  else {
